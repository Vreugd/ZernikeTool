...
def ZernikeDecomposition(rho,phi,m_max,dz,UnitFactor):
    ...
    for i in range(len(A)):
        ...
        ZernikeSum = np.sum(Zernikes,axis = 1)
        delta = dz - ZernikeSum
...
def PistonTipTiltTableFunc(Xlinear, PTT):
    ...
    for i in range(1,len(Xlinear)+2):
        PistonTable.append(' ')
    for i in range(3,len(Xlinear)+2):
        TipTiltTable.append(' ')
    ...
    if uploaded_file is not None:
        ...
        if ZernikeDecomposition_opt:
            ...
            with st.expander('Zernike Table'):
                ...
                SFEs = np.append(SFEs, ' ')
                SFEs = np.append(SFEs,  str(np.round(np.std(ZernikeDelta)*UnitFactor,3))    )
                ...
                if units == 'meters':
                    dfTable = pd.DataFrame({'Zernike Mode:' : ZernikeTable, 'PV [nm]' : PVs, 'SFE [nm RMS]:' : SFEs, 'Piston [m]:' : PistonTable, 'Tip Tilt angle [rad]:' : TipTiltTable}) 
                elif units == 'millimeters':
                    dfTable = pd.DataFrame({'Zernike Mode:' : ZernikeTable, 'PV [nm]' : PVs, 'SFE [nm RMS]:' : SFEs, 'Piston [mm]:' : PistonTable, 'Tip Tilt angle [rad]:' : TipTiltTable}) 
                st.write(dfTable) 
